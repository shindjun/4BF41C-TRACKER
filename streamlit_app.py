...
if st.button("계산하기"):
    ...
    # 누적 시간별 저선량 계산
    df_timeline['시간'] = df_timeline['출선종료']
    df_timeline['누적용선량'] = (output_radiation / avg_casts_per_day).cumsum()
    df_timeline['누적슬래그량'] = (slag_radiation / avg_casts_per_day).cumsum()
    df_timeline['누적총저선량'] = df_timeline['누적용선량'] + df_timeline['누적슬래그량']
    ...
    # 누적 저선량 시각화
    st.subheader("⏱️ 시간대별 누적 저선량 추이")
    fig2, ax2 = plt.subplots(figsize=(10, 4))
    ax2.plot(df_timeline['시간'], df_timeline['누적용선량'], label='누적 용선량')
    ax2.plot(df_timeline['시간'], df_timeline['누적슬래그량'], label='누적 슬래그량')
    ax2.plot(df_timeline['시간'], df_timeline['누적총저선량'], label='누적 총 저선량', linestyle='--')
    ax2.set_xlabel("시간 (분)")
    ax2.set_ylabel("ton")
    ax2.set_title("시간대별 누적 저선량 추이")
    ax2.legend()
    ax2.grid(True)
    st.pyplot(fig2)
