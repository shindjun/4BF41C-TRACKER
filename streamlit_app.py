...
coke_per_charge = 33
charges_per_day = 126
iron_rate = 9  # ton/min 평균 선철 생성속도
slag_ratio = 2.25
...
k_diameter = st.sidebar.number_input("출선구 비트경 D (파이, mm)", min_value=1.0, step=1.0)
if k_diameter > 0:
    k_result = k_speed / (k_diameter ** 2)
    st.sidebar.write(f"→ 환산계수 K = {k_result:.5f} ton/min·mm²")

st.sidebar.header("📊 자동 계산 항목")
total_ore = ore_per_charge * charges_per_day
total_coke = coke_per_charge * charges_per_day
total_ratio = total_ore / total_coke if total_coke > 0 else 0
hourly_charges = charges_per_day / 24
predicted_iron = iron_rate * 1440  # 24시간 기준
predicted_slag = predicted_iron / slag_ratio

st.sidebar.write(f"🪨 일일 Ore 장입량: {total_ore} ton")
st.sidebar.write(f"🔥 일일 Coke 장입량: {total_coke} ton")
st.sidebar.write(f"⚖️ 자동 계산 Ore/Coke 비율: {total_ratio:.2f}")
st.sidebar.write(f"⏱ 시간당 Charge 수: {hourly_charges:.2f} 회/hr")
st.sidebar.write(f"🧱 하루 출선량 (선철 생성속도 기준): {predicted_iron:.0f} ton")
st.sidebar.write(f"🌋 슬래그량 추정 (출선비 2.25): {predicted_slag:.0f} ton")

# --- 본문 입력값 ---
st.header("출선구별 비트경 입력")
tap_diameter_lead = st.number_input("선행 출선구 비트경 (파이, mm)", min_value=30.0, max_value=80.0, value=45.0, step=1.0)
tap_diameter_follow = st.number_input("후행 출선구 비트경 (파이, mm)", min_value=30.0, max_value=80.0, value=45.0, step=1.0)
...
def recommend_adjustments(casting_time):
    if casting_time > 300:
        return {"비트경 추천": "50mm 이상으로 확장 고려", "동시출선시간 조정": "30분 이상 확보 필요", "차기출선시간 조정": "대기시간을 10분 이내로 단축"}
    elif casting_time < 200:
        return {"비트경 추천": "현재 유지 가능 (예: 45mm)", "동시출선시간 조정": "기존 유지 또는 일부 단축 가능", "차기출선시간 조정": "기존 대기시간 유지"}
    else:
        return {"비트경 추천": "47~48mm로 미세 조정 고려", "동시출선시간 조정": "20~30분 확보", "차기출선시간 조정": "12분 수준 권장"}
...
if st.button("계산하기"):
    ...
    for idx, taphole in enumerate(taphole_labels):
        timeline.append({"출선차수": idx + 1, "출선구": taphole, "출선시작": current_time, "출선종료": current_time + cast_duration})
        current_time += cast_duration + switch_gap
    df_timeline = pd.DataFrame(timeline)

    lead_end_time = df_timeline[df_timeline['출선구'] == 'A'].iloc[0]['출선종료']
    ...
    for i, row in df_timeline.iterrows():
        ax.barh(row["출선구"], row["출선종료"] - row["출선시작"], left=row["출선시작"], height=0.4, color="tab:blue" if row["출선구"] == "A" else "tab:orange")
