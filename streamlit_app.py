...
furnace_lifetime = st.number_input("고로 수명 (년)", min_value=0)

tap_time = st.number_input("1개 출선구 평균 출선시간 (min)", min_value=0.0, value=default_tap_time)
tap_speed = st.number_input("출선 속도 (ton/min)", min_value=0.0, value=default_tap_speed)
tap_amount = st.number_input("1회 출선시 배출량 (ton)", min_value=0.0, value=default_tap_amount)
wait_time = st.number_input("차기 출선까지 대기 시간 (min)", min_value=0.0, value=default_wait_time)
